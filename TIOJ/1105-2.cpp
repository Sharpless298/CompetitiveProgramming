...
vector<Point> convex_hull(vector<Point> &pnts) {
	...
	vector<Point> hull;
	for(Point pnt : pnts) {
		while(hull.size()>=2 && cross(hull.back().first-hull[hull.size()-2].first, pnt.first-hull[hull.size()-2].first)<=0)
			hull.pop_back();
		hull.push_back(pnt);
	}
	hull.pop_back();
	...
	int t = (int)hull.size();
	for(Point pnt : pnts) {
		while(hull.size()-t>=2 && cross(hull.back().first-hull[hull.size()-2].first, pnt.first-hull[hull.size()-2].first)<=0)
			hull.pop_back();
		hull.push_back(pnt);
	}
	hull.pop_back();
...
signed main() {
	...
	while(cin>>n && n) {
		...
		auto f = [&](Point &a, Point &b) {
			...
			if(tmp == dis) ans = min({ans, {a.second, b.second}, {b.second, a.second}});
			if(tmp > dis) {
