...
long long f(string &s) {
	long long res = 0;
	for (int i = 0; i < n; i++) res = (res * P + s[i] - '0') % MOD;
	return res;
}
...
signed main() {
	...
	power[0] = 1;
	for (int i = 1; i < 100001; i++) power[i] = power[i - 1] * P % MOD;
	...
	while (T--) {
		...
		pref[0] = 0;
		for (int i = 0; i < n; i++) pref[i + 1] = (pref[i] * P + s[i] - '0') % MOD;
		suf[0] = 0;
		for (int i = 0; i < n; i++) suf[i + 1] = (suf[i] * P + s[n - i - 1] - '0') % MOD;
